...
// ── Status dot shown in header
function SyncStatus({ status }) {
  const cfg = {
    loading: { color:"#d97706", label:"Syncing…" },
    live:    { color:"#28a06a", label:"Live" },
    offline: { color:"#e05c3a", label:"Offline (data not saved)" },
  }[status] || {};
  return (
    <div style={{ display:"flex", alignItems:"center", gap:5, fontSize:11, color: cfg.color }}>
      <div style={{ width:7, height:7, borderRadius:"50%", background:cfg.color,
        boxShadow: status==="live" ? `0 0 0 2px ${cfg.color}44` : "none" }} />
      {cfg.label}
    </div>
  );
}
...
export default function CompoundingTracker() {
  ...
  // ── All persistent state lives here, synced to Supabase
  const { data, update, status } = useSync({
    compounds: DEFAULT_COMPOUNDS,
    schedule: {},
    inventory: {},
  });
  const { compounds, schedule, inventory } = data;
  const setCompounds = v => update("compounds", typeof v === "function" ? v(compounds) : v);
  const setSchedule  = v => update("schedule",  typeof v === "function" ? v(schedule)  : v);
  const setInventory = v => update("inventory", typeof v === "function" ? v(inventory) : v);
  ...
  const existingSupplyNames = useMemo(() => [...new Set(supplyCatalog.map(s=>s.name))], [supplyCatalog]);

  const getOnHand = s => inventory[s.name+"__"+s.unit] ?? 0;
  ...
  const setDayEntry = (dk, compId, field, val) =>
    setSchedule(s => ({ ...s, [dk]: { ...getDaySched(dk), [compId]: { ...((s[dk]||{})[compId]||{batches:1,unitsPerBatch:10}), [field]:val } } }));
  ...
  const tabBtn = active => ({
    padding:"9px 18px", fontSize:13, fontWeight:600, cursor:"pointer", border:"none", borderRadius:6, whiteSpace:"nowrap",
    background:active?"#1a2236":"transparent", color:active?"#fff":"#6b7280", transition:"all .15s",
  });
  const th = { padding:"9px 12px", fontSize:11, fontWeight:700, letterSpacing:".07em", textTransform:"uppercase",
    color:"#8892a4", borderBottom:"1.5px solid #e4e8ef", textAlign:"left", background:"#f8f9fb" };
    ...
    <div style={{ minHeight:"100vh", background:"#f0f2f7", fontFamily:"'Inter','Segoe UI',sans-serif" }}>
      ...
      <div style={{ background:"#1a2236", color:"#fff", padding:"0 28px" }}>
        <div style={{ maxWidth:1100, margin:"0 auto", display:"flex", alignItems:"center", justifyContent:"space-between", height:60 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:16 }}>
            ...
            <div style={{ fontSize:12, color:"#8fa3c4" }}>{compounds.length} compounds · {supplyCatalog.length} supplies · {poItems.length} to order</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ background:"#fff", borderBottom:"1.5px solid #e4e8ef", padding:"0 28px" }}>
        <div style={{ maxWidth:1100, margin:"0 auto", display:"flex", gap:4, padding:"8px 0", flexWrap:"wrap" }}>
          {[["schedule","📅 Schedule"],["calculator","⚗️ Calculator"],["compounds","✏️ Compounds"],["inventory","📦 Inventory"],["po","🛒 Purchase Order"]].map(([id,label])=>(
      ...
      <div style={{ maxWidth:1100, margin:"0 auto", padding:"24px 28px" }}>
          ...
          <div>
            ...
            <div style={{...card,marginBottom:20}}>
              <div style={{ display:"grid",gridTemplateColumns:"repeat(7,1fr)",borderBottom:"1.5px solid #e4e8ef" }}>
                {DAY_NAMES.map(d=><div key={d} style={{ padding:"8px 4px",textAlign:"center",fontSize:11,fontWeight:700,letterSpacing:".06em",textTransform:"uppercase",color:"#8892a4",background:"#f8f9fb" }}>{d}</div>)}
              </div>
              <div style={{ display:"grid",gridTemplateColumns:"repeat(7,1fr)" }}>
                {calCells.map((day,idx)=>{
                  ...
                  const dk=dateKey(calYear,calMonth,day), dayData=getDaySched(dk), compIds=Object.keys(dayData);
                  const isToday=dk===todayKey, isPast=new Date(calYear,calMonth,day)<new Date(today.getFullYear(),today.getMonth(),today.getDate());
                  return (
                    <div key={dk} onClick={()=>setSelectedDay(dk)}
                      style={{ minHeight:88,padding:"6px 6px 4px",cursor:"pointer",borderRight:"1px solid #f0f2f7",borderBottom:"1px solid #f0f2f7",background:isToday?"#fffbf0":isPast?"#fafafa":"#fff" }}>
                      <div style={{ fontSize:12,fontWeight:isToday?800:500,marginBottom:3,color:isToday?"#e05c3a":isPast?"#b0b8c8":"#374151",display:"flex",alignItems:"center",gap:4 }}>
                        {isToday&&<span style={{ width:5,height:5,borderRadius:"50%",background:"#e05c3a",display:"inline-block" }} />}{day}
                      </div>
                      <div style={{ display:"flex",flexDirection:"column",gap:2 }}>
                        {compIds.map(cid=>{
                          const comp=compounds.find(c=>c.id===cid); if(!comp) return null;
                          const e=dayData[cid], units=(e.batches||0)*(e.unitsPerBatch||0);
                          return <div key={cid} style={{ background:(comp.color||"#888")+"22",borderLeft:`3px solid ${comp.color||"#888"}`,border:`1px solid ${comp.color||"#888"}44`,borderRadius:3,padding:"2px 4px",fontSize:10,fontWeight:600,color:comp.color||"#888",lineHeight:1.4,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis" }}>{e.batches}×{e.unitsPerBatch}={units}u</div>;
                        })}
                    ...
                    return (
                      <div key={comp.id} style={{...card,marginBottom:0,padding:14,borderLeft:`4px solid ${comp.color}`}}>
                        ...
                        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:4 }}>
                          <div><div style={{ fontSize:10,color:"#8892a4",fontWeight:600,letterSpacing:".05em" }}>RUN DAYS</div><div style={{ fontSize:22,fontWeight:800,color:"#1a2236",fontFamily:"'JetBrains Mono',monospace" }}>{tot.runDays}</div></div>
                          <div><div style={{ fontSize:10,color:"#8892a4",fontWeight:600,letterSpacing:".05em" }}>TOTAL UNITS</div><div style={{ fontSize:22,fontWeight:800,color:"#1a2236",fontFamily:"'JetBrains Mono',monospace" }}>{tot.totalUnits}</div></div>
                        </div>
                      </div>
                    );
                  })}
        ...
        {activeTab==="calculator" && (
          <div>
            <p style={{ color:"#6b7280",fontSize:13,marginBottom:18 }}>Quick manual calculator. Inventory and PO use the Monthly Schedule totals.</p>
            {compounds.map(comp=>{
              const {batches:nb=1,unitsPerBatch:upb=10}=manualBatches[comp.id]||{}, totalUnits=nb*upb;
              return (
                <div key={comp.id} style={{...card,borderLeft:`4px solid ${comp.color||"#888"}`}}>
                  <div style={{ padding:"14px 18px",display:"flex",alignItems:"center",gap:14 }}>
                    <div style={{ flex:1 }}><div style={{ fontWeight:700,fontSize:14 }}>{comp.name}</div><div style={{ fontSize:12,color:"#8892a4",marginTop:2 }}>{comp.ingredients.length} ingredients · {comp.containers.length} containers</div></div>
                  ...
                  <div style={{ borderTop:"1px solid #f0f2f7" }}>
                    <table style={{ width:"100%",borderCollapse:"collapse" }}>
                      <thead><tr><th style={th}>Supply</th><th style={th}>Type</th><th style={{...th,textAlign:"right"}}>Per Unit</th><th style={{...th,textAlign:"right"}}>Total Needed</th></tr></thead>
                      <tbody>{[...comp.ingredients,...comp.containers].map(s=>(
                        <tr key={s.id}><td style={td}>{s.name}</td><td style={td}><Badge category={s.category}/></td>
                        <td style={{...td,textAlign:"right",fontFamily:"'JetBrains Mono',monospace"}}>{fmt(s.perUnit)} {s.unit}</td>
                        <td style={{...td,textAlign:"right",fontFamily:"'JetBrains Mono',monospace",fontWeight:700,color:"#1a2236"}}>{fmt(s.perUnit*totalUnits)} {s.unit}</td></tr>
                      ))}</tbody>
                    </table>
                  </div>
                </div>
        ...
        {activeTab==="compounds" && (
          <div>
            <p style={{ color:"#6b7280",fontSize:13,marginBottom:18 }}>Edit compound names, colors, and supply lists. Shared supplies are merged automatically in Inventory.</p>
            {compounds.map(comp=>{
              const isOpen=editingCompId===comp.id, allSupplies=[...comp.ingredients,...comp.containers];
              return (
                <div key={comp.id} style={{...card,borderLeft:`4px solid ${comp.color||"#888"}`}}>
                  <div style={{ padding:"14px 18px",display:"flex",alignItems:"center",gap:12,background:isOpen?"#f8f9fb":"#fff",cursor:"pointer" }}
                    ...
                    <div style={{ flex:1 }} onClick={e=>e.stopPropagation()}>
                      <EditableText value={comp.name} onChange={v=>updateCompound(comp.id,c=>({...c,name:v}))} style={{ fontWeight:700,fontSize:14 }} placeholder="Compound name" />
                      <div style={{ fontSize:12,color:"#8892a4",marginTop:3 }}>{comp.ingredients.length} ingredient{comp.ingredients.length!==1?"s":""} · {comp.containers.length} container item{comp.containers.length!==1?"s":""}</div>
                    </div>
                    <div style={{ display:"flex",alignItems:"center",gap:8 }}>
                      <button style={{...btn("danger"),fontSize:11,padding:"5px 12px"}}
                        onClick={e=>{e.stopPropagation();if(window.confirm(`Remove ${comp.name}?`)){setCompounds(cs=>cs.filter(c=>c.id!==comp.id));setEditingCompId(null);}}}>Remove</button>
                  ...
                  {isOpen && (
                    <div style={{ borderTop:"1.5px solid #e4e8ef" }}>
                      {allSupplies.length>0&&(
                        <table style={{ width:"100%",borderCollapse:"collapse" }}>
                          <thead><tr><th style={th}>Supply Name</th><th style={th}>Type</th><th style={{...th,textAlign:"right"}}>Per Unit</th><th style={{...th,width:36}}></th></tr></thead>
                          <tbody>{allSupplies.map(s=>(
                            <tr key={s.id} style={{ background:"#fff" }}>
                              <td style={td}><div style={{ display:"flex",alignItems:"center",gap:8 }}><EditableText value={s.name} onChange={v=>updateSupplyInComp(comp.id,s.id,"name",v)} style={{ fontSize:13,fontWeight:500 }} /><span style={{ fontSize:11,color:"#b0b8c8" }}>({s.unit})</span></div></td>
                              <td style={td}><select value={s.category} onChange={e=>updateSupplyInComp(comp.id,s.id,"category",e.target.value)} style={{ border:"1.5px solid #d4d8df",borderRadius:5,padding:"4px 6px",fontSize:12,background:"#f8f9fb",cursor:"pointer",outline:"none" }}><option value="api">Drug / API</option><option value="diluent">Diluent</option><option value="container">Container</option></select></td>
                              <td style={{...td,textAlign:"right"}}><div style={{ display:"flex",alignItems:"center",justifyContent:"flex-end",gap:4 }}><NumInput value={s.perUnit} min={0} onChange={v=>updateSupplyInComp(comp.id,s.id,"perUnit",v)} style={{ width:64 }} /><EditableText value={s.unit} onChange={v=>updateSupplyInComp(comp.id,s.id,"unit",v)} style={{ fontSize:12,color:"#6b7280",minWidth:30 }} /></div></td>
                              <td style={{...td,textAlign:"center",padding:"9px 8px"}}><button onClick={()=>removeSupplyFromComp(comp.id,s.id)} style={{ border:"none",background:"none",color:"#e05c3a",cursor:"pointer",fontSize:16,lineHeight:1,padding:2 }}>×</button></td>
                            </tr>
                          ))}</tbody>
                        </table>
                      )}
                      <div style={{ padding:14,background:"#f8f9fb",borderTop:"1px solid #f0f2f7" }}>
                        ...
                        <div style={{ display:"grid",gridTemplateColumns:"1fr 90px 80px 120px auto",gap:8,alignItems:"end" }}>
                          <div style={{ position:"relative" }}>
                            <input placeholder="Supply name" value={editNewSupply.name}
                            ...
                            {supplyNameFocus&&editNewSupply.name.length>0&&(
                              <div style={{ position:"absolute",top:"100%",left:0,right:0,zIndex:20,background:"#fff",border:"1.5px solid #d4d8df",borderRadius:6,boxShadow:"0 8px 20px rgba(0,0,0,.12)",maxHeight:160,overflowY:"auto" }}>
                                {existingSupplyNames.filter(n=>n.toLowerCase().includes(editNewSupply.name.toLowerCase())&&n!==editNewSupply.name).map(n=>{
                                  const existing=supplyCatalog.find(s=>s.name===n);
                                  return <div key={n} onMouseDown={()=>setEditNewSupply(s=>({...s,name:n,unit:existing?.unit||s.unit,category:existing?.category||s.category}))}
                                    style={{ padding:"8px 12px",cursor:"pointer",fontSize:13,display:"flex",alignItems:"center",gap:8,borderBottom:"1px solid #f0f2f7" }}
                                    onMouseEnter={e=>e.currentTarget.style.background="#f8f9fb"}
                                    onMouseLeave={e=>e.currentTarget.style.background="#fff"}>
                                    <Badge category={existing?.category||"api"}/><span>{n}</span><span style={{ fontSize:11,color:"#8892a4",marginLeft:"auto" }}>{existing?.unit}</span>
                                  </div>;
                                })}
                              </div>
                            )}
                          </div>
                          <input placeholder="Unit" value={editNewSupply.unit} onChange={e=>setEditNewSupply(s=>({...s,unit:e.target.value}))} style={inputStyle} />
                          <input type="number" placeholder="Per unit" min="0" step="any" value={editNewSupply.perUnit} onChange={e=>setEditNewSupply(s=>({...s,perUnit:Number(e.target.value)}))} style={inputStyle} />
                          <select value={editNewSupply.category} onChange={e=>setEditNewSupply(s=>({...s,category:e.target.value}))} style={{...inputStyle,cursor:"pointer"}}><option value="api">Drug / API</option><option value="diluent">Diluent</option><option value="container">Container</option></select>
                          <button style={{...btn(),padding:"8px 14px",whiteSpace:"nowrap"}} onClick={()=>addSupplyToComp(comp.id)}>+ Add</button>
                        </div>
                      </div>
                    </div>
                  )}
            ...
            {compounds.length===0&&<div style={{ textAlign:"center",padding:"48px 0",color:"#8892a4" }}>No compounds yet — use "+ Add Compound" above.</div>}
            {supplyCatalog.filter(s=>s.usedIn.length>1).length>0&&(
              <div style={{ marginTop:24 }}>
                <div style={{ fontWeight:700,fontSize:13,color:"#1a2236",marginBottom:10 }}>🔗 Shared Supplies</div>
                <div style={card}>
                  <table style={{ width:"100%",borderCollapse:"collapse" }}>
                    <thead><tr><th style={th}>Supply</th><th style={th}>Type</th><th style={th}>Used In</th></tr></thead>
                    <tbody>{supplyCatalog.filter(s=>s.usedIn.length>1).map(s=>(
                      <tr key={s.name+s.unit}>
                        ...
                        <td style={td}><div style={{ display:"flex",gap:6,flexWrap:"wrap" }}>{s.usedIn.map(name=>{const comp=compounds.find(c=>c.name===name);return <span key={name} style={{ fontSize:11,padding:"2px 8px",borderRadius:4,background:(comp?.color||"#888")+"22",color:comp?.color||"#555",fontWeight:600 }}>{name}</span>;})}</div></td>
                      </tr>
                    ))}</tbody>
                  </table>
                </div>
        ...
        {activeTab==="inventory" && (
          <div>
            {infoBanner(<>📅 Supply requirements for <strong>{MONTH_NAMES[calMonth]} {calYear}</strong>. Shared supplies show combined totals.</>)}
            {["api","diluent","container"].map(cat=>{
              ...
              return (
                <div key={cat} style={{ marginBottom:24 }}>
                  ...
                  <div style={card}>
                    <table style={{ width:"100%",borderCollapse:"collapse" }}>
                      <thead><tr><th style={th}>Supply</th><th style={th}>Used In</th><th style={{...th,textAlign:"right"}}>Needed (scheduled)</th><th style={{...th,textAlign:"right"}}>On Hand</th><th style={{...th,textAlign:"right"}}>Status</th></tr></thead>
                      <tbody>{supplies.map(s=>{
                        const sched=schedMap[s.name+"__"+s.unit], needed=sched?.needed||0, onHand=getOnHand(s), diff=onHand-needed;
                        return (
                          <tr key={s.name+s.unit}>
                            <td style={td}><div style={{ fontWeight:500 }}>{s.name}</div><div style={{ fontSize:11,color:"#8892a4" }}>{s.unit}</div></td>
                            <td style={td}><div style={{ display:"flex",gap:4,flexWrap:"wrap" }}>{s.usedIn.map(name=>{const comp=compounds.find(c=>c.name===name);return <span key={name} style={{ fontSize:10,padding:"1px 6px",borderRadius:3,background:(comp?.color||"#888")+"22",color:comp?.color||"#555",fontWeight:600 }}>{name}</span>;})}</div></td>
                            <td style={{...td,textAlign:"right",fontFamily:"'JetBrains Mono',monospace"}}>{needed>0?<><strong>{fmt(needed)}</strong> {s.unit}</>:<span style={{ color:"#c0c8d8" }}>—</span>}</td>
                            <td style={{...td,textAlign:"right"}}><div style={{ display:"flex",alignItems:"center",justifyContent:"flex-end",gap:6 }}><NumInput value={onHand} onChange={v=>setOnHand(s,v)} style={{ background:needed===0?"#f8f9fb":diff>=0?"#edfaf3":"#fff2ef",width:80 }} /><span style={{ fontSize:11,color:"#8892a4" }}>{s.unit}</span></div></td>
                            <td style={{...td,textAlign:"right"}}>{needed===0?<span style={{ fontSize:11,color:"#b0b8c8" }}>Not scheduled</span>:diff>=0?<span style={{ color:"#28a06a",fontWeight:700,fontSize:12 }}>✓ +{fmt(diff)}</span>:<span style={{ color:"#e05c3a",fontWeight:700,fontSize:12 }}>⚠ {fmt(diff)}</span>}</td>
                          </tr>
                        );
                      })}</tbody>
                    </table>
        ...
        {activeTab==="po" && (
          <div>
            ...
            {poItems.length===0?(
              <div style={{...card,padding:"48px 0",textAlign:"center",color:"#28a06a",fontWeight:600}}>✓ All supplies fully stocked for scheduled batches.</div>
            ):(
              <>
                <div style={card}>
                  <table style={{ width:"100%",borderCollapse:"collapse" }}>
                    <thead><tr><th style={th}>Supply</th><th style={th}>Type</th><th style={{...th,textAlign:"right"}}>Needed</th><th style={{...th,textAlign:"right"}}>On Hand</th><th style={{...th,textAlign:"right",color:"#e05c3a"}}>To Order</th></tr></thead>
                    <tbody>{["api","diluent","container"].map(cat=>poItems.filter(s=>s.category===cat).map(s=>(
                      <tr key={s.name+s.unit}>
                        <td style={td}><div style={{ fontWeight:500 }}>{s.name}</div><div style={{ fontSize:11,color:"#8892a4" }}>{s.compoundNames.join(", ")}</div></td>
                        <td style={td}><Badge category={s.category}/></td>
                ...
                <div style={{ display:"flex",justifyContent:"flex-end",gap:10 }}>
                  <button style={btn("ghost")} onClick={()=>{ const lines=poItems.map(s=>`${s.name}\t${fmt(s.toOrder)} ${s.unit}`).join("\n"); navigator.clipboard.writeText(`STERILE COMPOUNDING PURCHASE ORDER\n${MONTH_NAMES[calMonth]} ${calYear}\nDate: ${new Date().toLocaleDateString()}\n\nSupply\tQty to Order\n`+lines).catch(()=>{}); }}>📋 Copy</button>
                  <button style={btn()} onClick={()=>{ const rows=["Supply,Unit,Needed,On Hand,To Order",...poItems.map(s=>`"${s.name}",${s.unit},${fmt(s.needed)},${fmt(s.onHand)},${fmt(s.toOrder)}`)].join("\n"); const a=document.createElement("a"); a.href=URL.createObjectURL(new Blob([rows],{type:"text/csv"})); a.download=`PO_${calYear}-${String(calMonth+1).padStart(2,"0")}.csv`; a.click(); }}>⬇ Export CSV</button>
                </div>
              </>
            )}
          </div>
        ...
        <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,.45)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100 }}
          ...
          <div style={{ background:"#fff",borderRadius:14,width:520,maxHeight:"85vh",overflow:"auto",padding:28,boxShadow:"0 20px 60px rgba(0,0,0,.25)" }}>
            <div style={{ display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:20 }}>
              <div><div style={{ fontWeight:800,fontSize:17 }}>{new Date(selectedDay+"T12:00:00").toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric",year:"numeric"})}</div><div style={{ fontSize:12,color:"#8892a4",marginTop:2 }}>Schedule batch runs for this day</div></div>
                      ...
                      <div style={{ display:"flex",alignItems:"center",gap:10 }}>
                        {[["BATCHES","batches",entry.batches||1],["UNITS/BATCH","unitsPerBatch",entry.unitsPerBatch||10]].map(([label,field,val],i)=>(
                          <div key={field} style={{ display:"flex",alignItems:"center",gap:8 }}>
                            ...
                            <div><div style={{ fontSize:10,color:"#8892a4",fontWeight:600,letterSpacing:".05em",marginBottom:3 }}>{label}</div><NumInput value={val} min={1} onChange={v=>setDayEntry(selectedDay,compId,field,v)} /></div>
                          </div>
                        ))}
                        <div style={{ color:"#c0c8d8",fontSize:16,marginTop:14 }}>=</div>
                        <div style={{ marginTop:14 }}><span style={{ fontFamily:"'JetBrains Mono',monospace",fontWeight:800,fontSize:22,color:"#1a2236" }}>{units}</span><span style={{ fontSize:12,color:"#8892a4",marginLeft:4 }}>units</span></div>
                      </div>
                    </div>
                  );
                })}
                ...
                <div style={{ display:"flex",flexDirection:"column",gap:6 }}>
                  {compounds.filter(c=>!getDaySched(selectedDay)[c.id]).map(comp=>(
                    <button key={comp.id} onClick={()=>addCompToDay(selectedDay,comp.id)}
                      style={{ display:"flex",alignItems:"center",gap:10,padding:"9px 12px",border:`1.5px solid ${comp.color}44`,borderRadius:7,background:"#fff",cursor:"pointer",textAlign:"left" }}>
      ...
      {showAddModal&&(
        <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,.45)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100 }}>
          <div style={{ background:"#fff",borderRadius:14,width:560,maxHeight:"85vh",overflow:"auto",padding:28,boxShadow:"0 20px 60px rgba(0,0,0,.25)" }}>
            ...
            <div style={{ marginBottom:14 }}><label style={{ fontSize:12,fontWeight:600,color:"#6b7280",display:"block",marginBottom:5 }}>COMPOUND NAME</label><input placeholder="e.g. Morphine 1mg/mL 100mL Bag" value={newComp.name} onChange={e=>setNewComp(c=>({...c,name:e.target.value}))} style={inputStyle} /></div>
              ...
              <div style={{ marginBottom:14 }}>
                ...
                {[...newComp.ingredients,...newComp.containers].map(s=>(
                  <div key={s.id} style={{ display:"flex",alignItems:"center",gap:8,padding:"6px 0",borderBottom:"1px solid #f0f2f7" }}>
                    <Badge category={s.category}/><span style={{ fontSize:13,flex:1 }}>{s.name}</span>
                    <span style={{ fontSize:12,color:"#8892a4",fontFamily:"monospace" }}>{s.perUnit} {s.unit}/unit</span>
                    <button onClick={()=>setNewComp(c=>({...c,ingredients:c.ingredients.filter(i=>i.id!==s.id),containers:c.containers.filter(i=>i.id!==s.id)}))} style={{ border:"none",background:"none",color:"#e05c3a",cursor:"pointer",fontSize:16 }}>×</button>
                  </div>
                ))}
              </div>
            )}
            <div style={{ background:"#f8f9fb",borderRadius:8,padding:14,marginBottom:16 }}>
              ...
              <div style={{ display:"grid",gridTemplateColumns:"1fr 80px 80px 110px",gap:8,marginBottom:8 }}>
                <input placeholder="Supply name" value={newSupply.name} onChange={e=>setNewSupply(s=>({...s,name:e.target.value}))} style={inputStyle} />
                <input placeholder="Unit" value={newSupply.unit} onChange={e=>setNewSupply(s=>({...s,unit:e.target.value}))} style={inputStyle} />
                <input type="number" placeholder="Per unit" min="0" step="any" value={newSupply.perUnit} onChange={e=>setNewSupply(s=>({...s,perUnit:Number(e.target.value)}))} style={inputStyle} />
                <select value={newSupply.category} onChange={e=>setNewSupply(s=>({...s,category:e.target.value}))} style={{...inputStyle,cursor:"pointer"}}><option value="api">Drug / API</option><option value="diluent">Diluent</option><option value="container">Container</option></select>
              </div>
